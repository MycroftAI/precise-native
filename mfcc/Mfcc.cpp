...
Mfcc::Mfcc(int samplingRate, int numFilters, int binSize, int numCoeffs)
        : samplingRate(samplingRate), numFilters(numFilters), binSize(binSize), numCoeffs(numCoeffs) {
    ...
    for (int l = 0; l < numFilters; l++) {  // Filter parameters
